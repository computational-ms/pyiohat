...
class DeNovoBaseParser(BaseParser):
    ...
    def add_protein_ids(self):
        ...
        """
        import resource
        logger.info(f"MEMORY RSS before UPeptideMapper: {resource.getrusage(resource.RUSAGE_SELF).ru_maxrss / 1e6:.2f} GB")
    
        peptide_mapper = UPeptideMapper(self.params["database"])
        logger.info(f"MEMORY RSS after UPeptideMapper construction: {resource.getrusage(resource.RUSAGE_SELF).ru_maxrss / 1e6:.2f} GB")
    ...
    def get_meta_info(self):
        ...
        elif self.style in ("instanovo_style_1",):
            logger.warning(
                "This engine does not provide retention time information. Grouping only by MS_Level. This may cause problems when working with multi-file inputs."
            )

            # Build a separate ms_level-keyed lookup directly from the CSV
            ms_level_lookup = {}
            with open(self.params["rt_pickle_name"], mode="r") as meta_csv:
                meta_reader = csv.DictReader(meta_csv)
                for row in meta_reader:
                    rt = float(row["rt"])
                    if row["rt_unit"] in ("minute", "min"):
                        rt *= 60.0
                    spectrum_id = int(row["spectrum_id"])
                    ms_level = int(row["ms_level"])
                    precursor_mz = np.nan if row["precursor_mz"] == "" else float(row["precursor_mz"])
                    if spectrum_id not in ms_level_lookup:
                        ms_level_lookup[spectrum_id] = {}
                    ms_level_lookup[spectrum_id][ms_level] = [row["lineage_root"], precursor_mz, rt]

            ms2_keys = [sid for sid, levels in ms_level_lookup.items() if 2 in levels]
            instanovo_rows = {int(self.df.loc[idx, "spectrum_id"]): idx for idx in self.df.index}

            for scan_number, idx in instanovo_rows.items():
                if scan_number < len(ms2_keys):
                    sid = ms2_keys[scan_number]
                    meta = ms_level_lookup[sid][2]  # [filename, precursor_mz, rt]
                    self.df.loc[
                        idx,
                        ("raw_data_location", "exp_mz", "retention_time_seconds","spectrum_id"),
                    ] = [meta[0], meta[1], meta[2], sid]
                else:
                    logger.warning(
                        f"scan_number {scan_number} exceeds available MS2 spectra ({len(ms2_keys)}), skipping."
                    )        
        else:
    ...
    def process_unify_style(self):
        ...
        def log_mem(label):
            ...
            logger.info(f"MEMORY [{label}]: current={current/1e9:.2f}GB peak={peak/1e9:.2f}GB")
    
        self.df.drop_duplicates(inplace=True, ignore_index=True)
        log_mem("after drop_duplicates")
        self.clean_up_modifications()
        log_mem("after clean_up_modifications")
        self.assert_only_iupac_and_missing_aas()
        log_mem("after assert_iupac")
        self.add_protein_ids()
        log_mem("after add_protein_ids")  # <-- likely culprit
        self.get_meta_info()
        log_mem("after get_meta_info")
        self.calc_masses_offsets_and_composition()
        log_mem("after calc_masses")
        self.check_enzyme_specificity()
        log_mem("after enzyme_specs")
        self.add_ranks()
        log_mem("after add_ranks")
        self.add_decoy_identity()
        log_mem("after add_decoy")
        self.sanitize()
