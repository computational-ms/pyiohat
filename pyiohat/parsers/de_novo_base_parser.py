...
class DeNovoBaseParser(BaseParser):
    ...
    def _calc_mass(self, mz, charge):
        ...
        Returns:
            (pd.Series): mass
        """
        return mz.astype(float) * charge.astype(int) - (charge.astype(int) * self.PROTON)
    ...
    # 1. Build the master list using the dictionary-based rt_lookup structure
            ms2_entries = []
    # Sort by spectrum_id to ensure the positional mapping (0, 1, 2...) remains consistent
            for spec_id, meta_info in sorted(rt_lookup.items()):
        # Check if this specific entry is an MS2 spectrum
                if meta_info.get("ms_level") == 2:
            # Extract the values directly from the meta_info dictionary
                    rt_val = meta_info["rt"]
                    lineage_root = meta_info["lineage_root"]
                    precursor_mz = meta_info["precursor_mz"]

            # Pack it all together for positional access later
                    ms2_entries.append(
                        (spec_id, rt_val, lineage_root, precursor_mz, lineage_root)
                    )

            logger.info(
                f"Total ms2 entries: {len(ms2_entries)}, total InstaNovo rows: {len(self.df)}"
            )

    # 2. Map InstaNovo's 0-indexed sequence to the DataFrame's row index
    # We assume 'spectrum_id' in self.df is the index (0, 1, 2...)
            instanovo_rows = {
                int(self.df.loc[idx, "spectrum_id"]): idx for idx in self.df.index
            }

    # 3. Iterate and unpack directly from the master list using the sequence number
            for instanovo_seq_num, idx in instanovo_rows.items():
                if instanovo_seq_num < len(ms2_entries):
            # The instanovo_seq_num (0, 1, 2...) acts as the index for our master list
                    actual_spec_id, rt, lineage, mz, filename = ms2_entries[instanovo_seq_num]

            # Write the real metadata back to the DataFrame
                    self.df.loc[
                        idx,
                        [
                            "raw_data_location",
                    ...
                    ] = [lineage, mz, rt, actual_spec_id]
                else:
                    logger.warning(
                        f"InstaNovo sequence number {instanovo_seq_num} exceeds available MS2 spectra "
                        f"({len(ms2_entries)}), skipping."
                    )
        else:
